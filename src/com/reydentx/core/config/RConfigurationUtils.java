/*
 * Copied from org.apache.commons.configuration.ConfigurationUtils in apache commons-configuration-1.9 package
 */
 ...
 * @version $Id: ZConfigurationUtils.java 1208795 2011-11-30 21:18:17Z oheger $
 */
public final class RConfigurationUtils {
	 ...
	 */
	//mod by Zing
	//private static final Log LOG = LogFactory.getLog(ZConfigurationUtils.class); //orig
	private static final Logger LOG = Logger.getLogger(RConfigurationUtils.class);
	 ...
	 * configurations derived from {@code AbstractConfiguration} (e.g. list
	 * delimiters). For a full support of all of these features the
	 * {@code copy()} method of {@code AbstractConfiguration} should be used. In
	 * a future release this method might become deprecated.</p>
	 *
	 ...
	 * configuration. Properties in the source configuration are appended to the
	 * properties with the same key in the target configuration.</p>
	 * <p><em>Note:</em> This method is not able to handle some specifics of
	 * configurations derived from {@code AbstractConfiguration} (e.g. list
	 * delimiters). For a full support of all of these features the
	 * {@code copy()} method of {@code AbstractConfiguration} should be used. In
	...
	/**
	 * Converts the passed in {@code Configuration} object to a hierarchical one
	 * using the specified {@code ExpressionEngine}. This conversion works by
	 * adding the keys found in the configuration to a newly created
	 * hierarchical configuration. When adding new keys to a hierarchical
	 * configuration the keys are interpreted by its {@code ExpressionEngine}.
	 * If they contain special characters (e.g. brackets) that are treated in a
	 * special way by the default expression engine, it may be necessary using a
	 * specific engine that can deal with such characters. Otherwise <b>null</b>
	 * can be passed in for the {@code ExpressionEngine}; then the default
	 * expression engine is used. If the passed in configuration is already
	 * hierarchical, it is directly returned. (However, the
	 * {@code ExpressionEngine} is set if it is not <b>null</b>.) Otherwise all
	 * properties are copied into a new hierarchical configuration.
	 *
	 * @param conf the configuration to convert
	 ...
	 * in configuration object implements the {@code Cloneable} interface, its
	 * {@code clone()} method will be invoked. Otherwise an exception will be
	 * thrown.
	 *
	...
	/**
	 * Constructs a URL from a base path and a file name. The file name can be
	 * absolute, relative or a full URL. If necessary the base path URL is
	 * applied.
	 *
	 ...
	 * name. This method is called if the base path passed to {@code getURL()}
	...
	public static URL locate(FileSystem fileSystem, String base, String name) {
		if (LOG.isDebugEnabled()) {
			...
			buf.append("ZConfigurationUtils.locate(): base is ").append(base);
	 ...
	 * object. This method is called e.g. by the save() methods of file based
	 ...
	 * passed in file name is absolute. If this is the case, it is returned.
	 * Otherwise further checks are performed whether the base path and file
	 * name can be combined to a valid URL or a valid file name. <em>Note:</em>
	 * The test if the passed in file name is absolute is performed using
	 * {@code java.io.File.isAbsolute()}. If the file name starts with a slash,
	 * this method will return <b>true</b> on Unix, but <b>false</b> on Windows.
	 * So to ensure correct behavior for relative file names on all platforms
	 * you should never let relative paths start with a slash. E.g. in a
	 * configuration definition file do not use something like that:
	 * <pre>
	 * &lt;properties fileName="/subdir/my.properties"/&gt;
	 * </pre> Under Windows this path would be resolved relative to the
	 * configuration definition file. Under Unix this would be treated as an
	 * absolute path name.
	 *
	 * @param basePath the base path
	 * @param fileName the file name
	 ...
	 * file.toURI().toURL(). It was used to work around a bug in the JDK
	 ...
	 * details. Now that we switched to JDK 1.4 we can directly use
	 * file.toURI().toURL().
	 *
	 ...
	 * {@code JNDIConfiguration}. Per default such errors are simply logged and
	 * then ignored. This implementation will register a special
	 * {@link ConfigurationErrorListener} that throws a runtime exception
	 * (namely a {@code ConfigurationRuntimeException}) on each received error
	 * event.
	 *
	 * @param src the configuration, for which runtime exceptions are to be
	 * enabled; this configuration must be derived from {@link EventSource}
	 */
